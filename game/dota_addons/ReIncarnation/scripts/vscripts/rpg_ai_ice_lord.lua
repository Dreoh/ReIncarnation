...
function BehaviorLaunchOrb:Evaluate()
	self.orbAbility = thisEntity:FindAbilityByName("rpg_ice_lord_ice_blast")
	local target
	local desire = 0
...
function BehaviorAttackAncient:Evaluate()
	...
	for _,friend in pairs(allFriends) do
		if not friend:IsNull() and friend:IsAlive() and friend:GetUnitName() ~= "npc_dota_creature_ice_lord" then
